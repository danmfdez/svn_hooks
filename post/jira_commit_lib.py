        ...
        with requests.Session() as session:
            ...
            for v_key in issue_keys:
                ...
                if response.status_code == 200:
                    ...
                    sys.stderr.write( "[Error] {0} Issue Does Not Exist - {1}\n".format(v_key, response.text) )
                    return 1

            'Update issues'

            jira_rest_url = self.jira_server_url + "/rest/scmchangeset/1.0/changeset/activity"
            ...
            for v_key in issue_keys:
                ...
                if response.status_code == 200:
                    ...
                    if issue.get("key"):
                        ...
                        sys.stderr.write( "[Warn] issue {0} not exists.\n".format(v_key) )
                else:
                    sys.stderr.write( "[Warn] {0} Issue Does Not Exist - {1}\n".format(v_key, response.text) )

            if index == 0:
                sys.stderr.write( "[Error] *** Required at-least one valid JIRA Issue ID. e.g. [JIRA DUMMY-5].\n" )
                sys.exit(1)
